...
public enum BalanceInqRq {
    ...
    ADDITIONAL_PRIVATE_DATA(ISOField.ADDITIONAL_PRIVATE_DATA.getPosition()),
    TRANSACTION_CURRENCY_CODE(ISOField.TRANSACTION_CURRENCY_CODE.getPosition(), false),
    PIN_DATA(ISOField.PIN_DATA.getPosition()),
    MAC_CODE(ISOField.MAC_CODE.getPosition());

    private Integer position;
    private boolean required;

    BalanceInqRq(Integer position) {
        this.position = position;
        this.required = true;
    }

    BalanceInqRq(Integer position, boolean required) {
        this.position = position;
